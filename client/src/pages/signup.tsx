...
const SIGNUP_URL = "http://localhost:8080/v1/auth/users";
...
export default function SignupPage() {
    ...
    const mutation = useMutation({
        mutationKey: ["user"],
        mutationFn: async ({ username, password, email }: Form) =>
            await http(SIGNUP_URL).post({
                username,
                email,
                password,
            }),
    ...
    async function onSubmit({ username, email, password }: Form) {
        mutation.mutate({
